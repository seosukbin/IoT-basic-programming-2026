...
- extern을 사용할때는 static 전역 변수를 사용 해서는 안된다
...
1. static
   - static은 정적이라는 의미로 소스 코드가 동작 하는 동안 끝까지 살아 남는 변수나 함수를 말한다.
   - static으로 선언하면 자기 소스 내에서는 위와 같이 동작한다.
   - 외부 소스로는 공유 하지 않는다.  <-> 객체지향 언어와는 정반대 이다.
   - 외부 소스에 공개 하지 않을 변수를 static으로 지정할것
   - ``두 소스에 같은 이름의 함수나 변수를 쓰는데 중복 문제를 해결 하기 위해서 사용``
   - 